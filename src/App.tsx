      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-4 p-3 sm:p-4 lg:p-6 max-w-[1700px] mx-auto w-full">
        ...
        <div className="flex-1 min-w-0 lg:min-h-0 flex flex-col gap-4">
          ...
          <div className="flex-1 lg:min-h-0 flex flex-col lg:flex-row gap-3 lg:gap-4">
            {/* Board. The eval bar is positioned as an absolute overlay to
                the LEFT of the board (outside it) so the board's layout is
                identical whether analysis is on or off — no grid layout
                that could collapse if a parent has 0 width. */}
            ...
            {/* Right column. PlaybackControls and MoveNote are visible on both
                desktop and mobile (high-priority). The Moves/Idea tabs only
                show inline on desktop; on mobile they open as bottom sheets
                triggered by MobileActions. */}
